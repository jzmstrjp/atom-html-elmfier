...
export default {
  ...
  makeAttrString(attributes) {
    const attrStringUnits = Array.from(attributes).map(key => `${this.formatAttrName(key.nodeName)} ${this.formatAttrVal(key.nodeValue)}`);
  ...
  formatAttrName(attrName) {
    const name = (attrName === 'type')
      ? `${attrName}_` : attrName;
    return name;
  },

  formatAttrVal(attrVal) {
    const value = /^[1-9](|([0-9]*?))$/.test(attrVal)
      ? attrVal : `"${attrVal}"`;
    return value;
  },
